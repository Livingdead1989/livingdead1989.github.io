...
| **Forwarding Database** | **Routing Table**   | *List of routes generated when an algorithm is run on the link-state database.* The routing table of each router is unique and contains information on how and where to send packets to other routers. <br/>* Can be viewed using the **show ip route** command. |
