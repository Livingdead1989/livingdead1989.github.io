...
<div class="mermaid">
pie title Server Storage
    "Logs" : 40
    "Backups of Backups" : 25
    "Stuff Nobody Remembers Deploying" : 20
    "The Actual App" : 15 
</div>
<div class="mermaid-caption">Storage utilisation as explained during postmortems </div>
...
<div class="mermaid"> 
sequenceDiagram 
  User ->> Web: Clicks Button 
  Web ->> API: Please do the thing 
  API -->> Auth: Is this user allowed? 
  Auth -->> API: Probably 
  API ->> Database: Query 
  Database -->> API: Slow response 
  API ->> Cache: Do you have this? 
  Cache --x API: Nope 
  API -->> User: 500 Internal Server Error
</div>
<div class="mermaid-caption">Normal production request lifecycle </div>
...
<div class="mermaid"> 
  flowchart LR 
    A((Monitoring Alert)) --> B{Is it on fire?} 
    B -- Yes --> C[Restart Service]
    B -- No ----> E
    C --> D{Did that fix it?} 
    D -- Yes --> E[Close Ticket] 
    D -- No --> F[Blame DNS] 
</div>
<div class="mermaid-caption">Its always DNS</div>
...
<div class="mermaid-caption">It solves my problem, it might solve yours!</div>
...
timeline 
  ...
  2020 : Temporary setup 
  2021 : Still temporary 
  2022 : Added monitoring 
  2023 : Added more monitoring 
  2024 : Decided it's "production" 
</div>
<div class="mermaid-caption">Nothing is more permanent than a temporary solution </div>
...
<figure class="media">
  <!-- mobile prevent fullscreen using playsinline -->
  <video 
    controls
    playsinline
    webkit-playsinline
